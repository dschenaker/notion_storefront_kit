// app.js — shop without sidebar, dedicated Categories page, image fail badge
(() => {
  'use strict';
  ...
  async function loadProducts(){
    ...
    products = json.map(p => ({
      ...
      images: Array.isArray(p.images) && p.images.length ? p.images.filter(Boolean) : (p.image ? [p.image] : [])
    }));
  }
  ...
  // UI
  function logoHTML(p){
    if (p.logo) return `<div class="logo-badge"><img alt="" src="${esc(ver(p.logo))}" loading="lazy" onerror="this.style.display='none'"></div>`;
    return `<div class="logo-badge"><span class="logo-fallback">${esc(initials(p.name))}</span></div>`;
  }

  function onImgErrorJS(key=''){
    // simple inline handler factory
  ...
  function renderCardsInto(el, items){
    if (!el) return;
    el.innerHTML = (items||[]).map(p=>{ try { return cardHTML(p); } catch(e){ console.error('Card error', p?.id, e); return ''; } }).join('')
      || `<p class="hint" style="padding:16px">No products.</p>`;
  }
        ...
        <div class="cat-grid">
          ${cats.map(c=>`
            <a class="cat-card" href="#/category/${esc(c.slug)}" aria-label="${esc(c.name)}">
              ${c.hero ? `<img class="cat-hero" src="${esc(ver(c.hero))}" alt="${esc(c.name)}" onerror="${onImgErrorJS('cat-hero')}">`
  ...
  function renderProduct({ id }){
      ...
      <div class="product-page">
        <div class="gallery">
          <div class="gallery-main ${fit ? 'fit' : ''}">
            ${gallery.length ? `<img id="gMain" src="${esc(ver(gallery[0]))}" alt="${esc(p.name)}" loading="eager" onerror="${onImgErrorJS('product-main')}">`
          ...
          <div class="variant-picker" id="variantPicker" role="tablist" aria-label="Variants">
            ${gallery.map((u,i)=>`
              <button class="variant ${i===0?'active':''}" data-i="${i}" role="tab" aria-selected="${i===0?'true':'false'}">
                <img src="${esc(ver(u))}" alt="Variant ${i+1}" onerror="${onImgErrorJS('variant')}">
              </button>`).join('')}
          </div>` : ``}
        </div>
    ...
    function applyFit(){ mainWrap.classList.toggle('fit', fit); if (viewBtn) viewBtn.textContent = `View: ${fit ? 'Fit' : 'Fill'}`; }
    viewBtn?.addEventListener('click', ()=>{ fit=!fit; localStorage.setItem('storefront_view_fit', fit?'1':'0'); applyFit(); });
    applyFit();
  ...
  function wireCardButtons(){
    ...
    document.querySelectorAll('[data-buy]').forEach(btn=>{
      btn.onclick = ()=>{ addToCart(btn.getAttribute('data-buy')); alert('Added to cart (no payment link on this item).'); };
    });
    updateCartUI();
  }
